...
## Fase 2 — Análise Semântica e Geração de Código MEPA

### Análise Semântica implementada

| Regra | O que verifica |
|---|---|
| **Declaração prévia** | Variáveis/funções usadas em expressões e atribuições devem estar declaradas |
| **Unicidade** | Redeclaração no mesmo escopo é detectada e reportada |
| **Escopo** | `ts_lookup` percorre do mais interno ao global; escopo correto atribuído a cada símbolo |
| **Checagem de tipos** | Atribuições (`int` ← `bool` é erro), operadores (`+` exige `int`, `^`/`v`/`~` exigem `bool`), condições de `if`/`loop` devem ser `bool` ou `int` |
| **Validação de parâmetros** | Quantidade de argumentos verificada contra `extra` do símbolo da sub-rotina |

### Geração de código MEPA

O arquivo `.mepa` é gerado automaticamente junto à compilação (mesmo nome do fonte, extensão `.mepa`).
...
| `AMEM n` / `DMEM n` | Aloca / desaloca `n` posições na pilha |
| `CRCT k` | Empilha constante `k` |
| `CRVL n,d` | Empilha valor da variável (nível `n`, deslocamento `d`) |
| `ARMZ n,d` | Desempilha e armazena na variável (nível `n`, deslocamento `d`) |
| `SOMA` / `SUBT` / `MULT` / `DIVI` | Operações aritméticas (notação pós-fixa) |
| `CMIG` / `CMDG` / `CMAI` / `CMME` / `CMAG` / `CMEG` | Comparações (empilham 0 ou 1) |
| `CONJ` / `DISJ` / `NEGA` | Operações lógicas AND / OR / NOT |
| `DSVF L` | Desvia para `L` se topo = falso (0) |
| `DSVS L` | Desvio incondicional para `L` |
| `NADA` | Instrução vazia — alvo de desvios |
| `TROC` | Troca os dois elementos do topo |
| `LEIT` | Lê inteiro do teclado e empilha |
| `IMPR` | Desempilha e imprime o valor |
| `PARA` | Encerra a execução |

#### Estratégia de endereçamento

```
CRVL nivel,deslocamento
ARMZ nivel,deslocamento
```

- `nivel 0` → variáveis globais (`globals`)
- `nivel 1` → variáveis locais do `proc main` (incluindo `locals`)

#### Estruturas de controle

**if-else:**
```
<expr>  DSVF L_else  <cmd_then>  DSVS L_fim
L_else: NADA  <cmd_else>  L_fim: NADA
```

**loop while:**
```
L_ini: NADA  <expr>  DSVF L_fim  <cmd>  DSVS L_ini
L_fim: NADA
```

**loop..until:**
```
L_ini: NADA  <cmds>  <expr>  DSVF L_ini
```

**for (implementado como loop while):**
```
ctrl := ini  ARMZ
L_ini: NADA
ctrl  fim  TROC  CMME  DSVF L_fim
<cmd>
ctrl := ctrl + passo  ARMZ  DSVS L_ini
L_fim: NADA
```

#### Limitações da geração MEPA (conforme Apêndice B)
- Sub-rotinas (`fn`, `proc`) são aceitas sintaticamente e verificadas semanticamente, mas **não geram código MEPA**
- Vetores são aceitos sintaticamente e semanticamente, mas **não geram código MEPA**
- Strings em `print` são aceitas mas não têm instrução equivalente na MEPA
