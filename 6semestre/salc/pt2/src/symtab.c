...
/* ------------------------------------------------------------------ */
/* Funcoes adicionadas para a fase 2 (semantica + geracao MEPA)         */
/* ------------------------------------------------------------------ */

int ts_nivel_atual(void) {
    return g_top; /* 0 = global, 1 = sub-rotina, 2 = locals, etc. */
}
...
int ts_count_current(void) {
    ...
    int n = 0;
    for (Symbol *s = g_stack[g_top].head; s; s = s->next) {
        if (s->cat == CAT_VAR || s->cat == CAT_PARAM) n++;
    }
    return n;
}
...
int ts_count_global(void) {
    if (g_top < 0) return 0;
    /* O escopo global esta sempre na base (indice 0) */
    int n = 0;
    for (Symbol *s = g_stack[0].head; s; s = s->next) {
        if (s->cat == CAT_VAR) n++;
    }
    return n;
}
...
/*
 * ts_insert_in_parent – insere um simbolo no escopo imediatamente
 * anterior ao corrente (o "pai"). Usado para registrar sub-rotinas
 * no escopo que as contém sem fechar o escopo corrente.
 */
Symbol *ts_insert_in_parent(const char *name, SymCat cat,
                             SymType type, int extra, int line) {
    if (g_top < 1) {
        /* Sem pai: insere no corrente */
        return ts_insert(name, cat, type, extra, line);
    }

    /* Verifica redeclaracao no escopo pai */
    int pai = g_top - 1;
    for (Symbol *s = g_stack[pai].head; s; s = s->next) {
    ...
    /* Caminho de escopo: usar o pai */
    char path[MAX_SCOPE * MAX_SCOPE_DEPTH];
    path[0] = '\0';
    for (int i = 0; i <= pai; i++) {
        if (i > 0) strncat(path, ".", sizeof(path) - strlen(path) - 1);
        strncat(path, g_stack[i].name, sizeof(path) - strlen(path) - 1);
    }
    strncpy(sym->scope, path, MAX_SCOPE - 1);
    sym->scope[MAX_SCOPE - 1] = '\0';

    /* Insere no pai */
    if (!g_stack[pai].tail) {
        g_stack[pai].head = g_stack[pai].tail = sym;
    } else {
        g_stack[pai].tail->next = sym;
        g_stack[pai].tail = sym;
    }
    return sym;
}
...
void ts_fix_params(int nparams, int nivel) {
    ...
    int idx = 0;
    for (Symbol *s = g_stack[g_top].head; s; s = s->next) {
        if (s->cat != CAT_PARAM) continue;
        idx++;
        s->nivel = nivel;
        s->addr = -(nparams + 5 - idx);
    }
}
